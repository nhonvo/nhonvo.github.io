...
This approach uses a custom HTTP request header (e.g., `X-Api-Version`) to specify the version. This is often favored by REST purists because the URI (`/api/products`) remains a canonical identifier for the resource, and the header simply requests a different _representation_ of that resource.
...
## Answer (API Versioning Implementation in C#)
