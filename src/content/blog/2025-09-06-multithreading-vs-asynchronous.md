...
description: "Distinguish between CPU-bound and I/O-bound operations and when to use threads vs async/await."
pubDate: "9 6 2025"
published: true
tags:
  [
...
Write a method that processes a list of 100 items. Implement three versions:

1.  A simple for-loop.
2.  Using `Parallel.ForEach`.
3.  Using a list of `Task`s with `Task.WhenAll`.

For each item, simulate both I/O-bound work (`Task.Delay`) and CPU-bound work (a tight loop). Discuss the performance and resource usage of each approach.

## Answer

### Code Example (C# Implementation)
...
3.  **`Task.WhenAll`**: This will be the fastest and most efficient approach for this specific workload.
    ...
    - During the `await Task.Delay(50)` part of each task, the threads are released and are free to do other work.
